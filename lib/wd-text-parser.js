...
module.exports = function() {
 ...
 let matchAndReplace = function(pattern, subject) {
  ...
  let parseOccupations = function(occupations) {
    ...
    let occupationLevels = getOccipationLevels(occupations);
    let singleLevelOccupaitons = occupationLevels.singleLevelOccupaitons;
    let multilevelOccupations = occupationLevels.multilevelOccupations;

    if (multilevelOccupations != null && multilevelOccupations.length > 0) {
      console.log('multilevelOccupations:\n'+ multilevelOccupations.join('\n'));
      ...
      if (occupationGroups !== null && occupationGroups.length > 0) {
        occupationGroups.forEach(function(occupationGroup) {
           occupationsObj.push(parseOccupationGroupRecursive(occupationGroup));
        });
      }
    }

    if (singleLevelOccupaitons.length > 0) {
      // Occupation and have depth {1}
      console.log('SinglelevelOccupations:\n'+ singleLevelOccupaitons.join('\n'));
      let flatOccupationsContent = singleLevelOccupaitons.join('\n');
      let flatOccupations = flatOccupationsContent.match(WD_WG_OCCUPATION);
      if (flatOccupations != null && flatOccupations.length > 0) {
        flatOccupations.forEach(function(flatOccupation) {
         occupationsObj.push(parseOccupation(flatOccupation));
       });
      }
    }

    // let flatOccupationsContent = occupations;
    // // Occupation Groups and it can have depth {2,3}
    // let occupationGroups = occupations.match(WD_WG_OCCUPATION_GROUP);
    // if (occupationGroups !== null && occupationGroups.length > 0) {
    //   occupationGroups.forEach(function(occupationGroup) {
    //      occupationsObj.push(parseOccupationGroupRecursive(occupationGroup));
    //   });
    //   flatOccupationsContent = occupations.replace(WD_WG_OCCUPATION_GROUP, '');
    // }
    return occupationsObj;
  };

  let getOccipationLevels = function(occupations) {
      let allLinesOfOccupations = occupations.match(/^.*[\S\s]$/gmi);
      let singleLevelOccupaitons = [];
      let singleLevelQualifiedBucket = [];
      let multilevelOccupations = [];
      if (allLinesOfOccupations.length !== null && allLinesOfOccupations.length > 1) {
        for(let i = 0; i < allLinesOfOccupations.length; i++) {
          // console.log('spaces:'+countNumberOfWhiteSpacesBeforeStartOfText(allLinesOfOccupations[i])+allLinesOfOccupations[i]);
          ...
          if(spaceCountLine == 0) {
            ...
            if (currentLine.match(/^.*\.+\$.*/gmi) != null) {
                singleLevelOccupaitons.push(singleLevelQualifiedBucket.join(''));
      ...
      return {
        'singleLevelOccupaitons': singleLevelOccupaitons,
        'multilevelOccupations': multilevelOccupations,
      };
  };
  let parseFlatOccupations = function(flatOccupationsContent) {
    // Occupation and have depth {1}
    let flatOccupationsObj = [];
    let flatOccupations = flatOccupationsContent.match(WD_WG_OCCUPATION);
    if (flatOccupations != null && flatOccupations.length > 0) {
      flatOccupations.forEach(function(flatOccupation) {
       flatOccupationsObj.push(parseOccupation(flatOccupation));
     });
    }
    return flatOccupationsObj;
  };

  let parseOccupationGroupRecursive = function(occupationGroup) {
    occupationGroup = reformatText(occupationGroup);
    ...
    let occupationGroupName = match(groupNameRegex, occupationGroup);
    let subOccupationsContent = '';
    if (occupationGroupName !== null) {
      subOccupationsContent = occupationGroup.replace(groupNameRegex, '');
    } else {
      console.log('levels: '+ levels);
      console.log('UnMatched :\n'+ occupationGroup);
      console.log('occupationGroup:\n'+occupationGroup);
    }


     if (levels.length > 2) {
       let regexString = '(^\\s{' + levels[1] + '}[\\w\\W]*?)(^\\s{' + levels[2] + '}[\\w\\W]*?\\n)\+';
       var regex = new RegExp(regexString, 'gmi');
       let subRates = [];
       occupationGroup.match(regex).forEach(function(matched) {
         subRates.push(parseOccupationGroupRecursive(matched));
       });
       return {
      ...
      return {
        'title': occupationGroupName,
        'rates': parseFlatOccupations(subOccupationsContent),
        'isGroup': true,
      };
  };

  let parseOccupation = function(occupationRate) {
     ...
     if (occupationFields.length == 2) {
        ...
        let rate = match(/\$\s+\d+.\d+(?=(\s+\d*.?\d*)?)/gmi, fringeraterow);
