...
let parseWageGroup = function(wageGroup) {
  ...
  let wageGroupOccupationsContent = wageGroup;
  let rates = wageGroup.match(/.*\.+\$.*/gmi);
  if (rates !== null && rates.length > 0) {
    let lastRate = rates[rates.length -1];
    let indexOfLastRate = wageGroup.lastIndexOf(lastRate);
    let indexOfWageGroupQualifier = -1;
    if (Number.isInteger(indexOfLastRate)) {
      indexOfWageGroupQualifier = indexOfLastRate + lastRate.length;
    }
    wageGroupQualifier = wageGroup.substring(indexOfWageGroupQualifier).replace(/\-{64,}/gmi,'').trim();
    wageGroupOccupationsContent = wageGroup.substring(0, indexOfWageGroupQualifier);
  }
  ...
  if (multilevelOccupations.length > 0) {
    ...
    if (occupationGroups !== null && occupationGroups.length > 0) {
      occupationGroups.forEach(function(occupationGroup) {
        ...
        let groupNameRegexString = '^\\S[\\s\\S]*?(?=^(\\s{' + spacecount + '}.*)+)';
