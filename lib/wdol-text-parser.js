...
module.exports = function(fileName) {
  ...
  const WD_WG_OCCUPATION = /^\s{0,2}\w[\s\S]*?\.+\$.*/gmi;
    ...
    var parseWageDeterminationTextFile = function(filePath, callback) {
    ...
    let parseHeaderIfo = function(content) {
      // const GENERAL_DECISION_KEY = 'General Decision Number';
      // const CONSTRUCTION_TYPES_KEY = 'Construction Types';
      // const COUNTIES_KEY = 'Counties';
      // const STATE_KEY = 'State';

      // let headerRows = WD_HEADER_INFORMATION[Symbol.match](content).join('\n');
      ...
      //   if (fields.length == 2) {
      //      if (fields[0] === GENERAL_DECISION_KEY) {
      //        headerObj.wagedeterminationCode = fields[1];
      //      } else if (fields[0] === COUNTIES_KEY) {
      //        headerObj.counties = content.match(/^count(y|ies):[\s\S]*?\./gmi).join('').replace(/^count(y|ies):/gmi, '').trim();
      //      } else if (fields[0] === STATE_KEY) {
      //        headerObj.state = fields[1];
      //      } else if (fields[0] === CONSTRUCTION_TYPES_KEY) {
      //        headerObj.constructionTypes = fields[1];
      //      }
      //        }
      //      });
      //    }
  ...
  return {
    getWageDetermination: parseWageDeterminationTextFile,
    getHeaderInformation: parseHeaderIfo,
    getModifications: parseModifications,
